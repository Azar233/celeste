use bevy::prelude::*;

use crate::audio::{play_dash_sfx, play_death_sfx};
use crate::components::{
    ClimbStamina, ClimbTopOutState, ColliderSize, CornerBoostState, Crouching, DashSlideState,
    DashState, Facing, FreezeFrameState, Ground, Grounded, JumpSource, JumpState, MovementInput,
    Player, PlayerActionInput, PlayerState, PlayerStateMachine, Velocity, WallContact,
    WallJumpTimer,
};
use crate::constants::{
    AIR_ACCEL, AIR_FRICTION, AIR_TURN_FRICTION,
    CLIMB_STAMINA_HOLD_DRAIN_PER_SEC, CLIMB_STAMINA_JUMP_COST, CLIMB_STAMINA_MAX,
    CLIMB_STAMINA_MIN_TO_CLIMB, CLIMB_STAMINA_UP_DRAIN_PER_SEC, CORNER_BOOST_BONUS_SPEED,
    CORNER_BOOST_MIN_BLOCKED_SPEED, CORNER_BOOST_TOP_TOLERANCE, CORNER_BOOST_WINDOW,
    CROUCH_COLLIDER_SIZE, DASH_CORNER_CORRECTION, DASH_DURATION, DASH_END_MULTIPLIER,
    DASH_FREEZE_TIME, DASH_SLIDE_SPEED_MULTIPLIER, DASH_SLIDE_WINDOW, DASH_SPEED, DEATH_THRESHOLD,
    DUCK_SUPER_JUMP_X_MULTIPLIER, DUCK_SUPER_JUMP_Y_MULTIPLIER, GRAVITY, GROUND_ACCEL,
    GROUND_FRICTION, GROUND_TURN_FRICTION, HALF_GRAVITY_THRESHOLD, JUMP_AWAY_FROM_WALL,
    JUMP_BUFFER_TIME, JUMP_GRACE_TIME, JUMP_VELOCITY, MAX_FALL_SPEED, MAX_RUN_SPEED,
    PLAYER_COLLIDER_SIZE, PLAYER_FALL_GRAVITY_MULTIPLIER, PLAYER_FALL_HELD_GRAVITY_MULTIPLIER,
    PLAYER_JUMP_HELD_GRAVITY_MULTIPLIER, PLAYER_JUMP_RELEASED_GRAVITY_MULTIPLIER,
    SPRING_ASCEND_GRAVITY_MULTIPLIER, SPRING_FALL_GRAVITY_MULTIPLIER,
    SPRING_FALL_HELD_GRAVITY_MULTIPLIER, SUPER_JUMP_APEX_GRAVITY_MULTIPLIER,
    SUPER_JUMP_APEX_THRESHOLD, SUPER_JUMP_FALL_MULTIPLIER, SUPER_JUMP_GRAVITY_WINDOW,
    SUPER_JUMP_HORIZONTAL_SPEED, SUPER_JUMP_TURN_FRICTION, SUPER_JUMP_VERTICAL_SPEED,
    UPWARD_CORNER_CORRECTION, WALL_CLIMB_JUMP_FORCE_Y, WALL_CLIMB_LOCK, WALL_CLIMB_SPEED,
    WALL_KICK_FORCE, WALL_KICK_LOCK, WALL_NEUTRAL_FORCE, WALL_NEUTRAL_LOCK, WALL_SLIDE_SPEED,
};
use crate::level::ActiveRoom;
use crate::systems::level::DeathSequence;
use crate::utils::{can_use_collider, check_collision, move_towards};

fn try_consume_ground_jump(
    jump_state: &mut JumpState,
    grounded: &Grounded,
    velocity: &mut Velocity,
) -> bool {
    if jump_state.jump_buffer_timer > 0.0
        && (grounded.0 || jump_state.jump_grace_timer > 0.0)
        && jump_state.spring_lock_timer <= 0.0
    {
        velocity.0.y = JUMP_VELOCITY;
        jump_state.jump_grace_timer = 0.0;
        jump_state.jump_buffer_timer = 0.0;
        jump_state.super_jump_timer = 0.0;
        jump_state.fast_jump_active = false;
        jump_state.source = JumpSource::Player;
        return true;
    }

    false
}

fn try_consume_super_jump(
    jump_state: &mut JumpState,
    dash_slide: &mut DashSlideState,
    velocity: &mut Velocity,
) -> bool {
    if jump_state.jump_buffer_timer > 0.0
        && dash_slide.timer > 0.0
        && jump_state.spring_lock_timer <= 0.0
    {
        perform_super_jump(jump_state, velocity, dash_slide.direction, true);
        dash_slide.timer = 0.0;
        return true;
    }

    false
}

fn perform_super_jump(
    jump_state: &mut JumpState,
    velocity: &mut Velocity,
    direction: f32,
    ducking: bool,
) {
    velocity.0.x = direction.signum() * SUPER_JUMP_HORIZONTAL_SPEED;
    velocity.0.y = SUPER_JUMP_VERTICAL_SPEED;

    if ducking {
        velocity.0.x *= DUCK_SUPER_JUMP_X_MULTIPLIER;
        velocity.0.y *= DUCK_SUPER_JUMP_Y_MULTIPLIER;
    }

    jump_state.jump_grace_timer = 0.0;
    jump_state.jump_buffer_timer = 0.0;
    jump_state.super_jump_timer = SUPER_JUMP_GRAVITY_WINDOW;
    jump_state.fast_jump_active = true;
    jump_state.source = JumpSource::Player;
}

fn try_consume_grounded_super_jump(
    jump_state: &mut JumpState,
    dash_state: &mut DashState,
    grounded: &Grounded,
    move_input: &MovementInput,
    velocity: &mut Velocity,
) -> bool {
    if jump_state.jump_buffer_timer <= 0.0
        || !grounded.0
        || move_input.y >= 0.0
        || !dash_state.is_dashing
        || dash_state.direction.x == 0.0
        || dash_state.direction.y != 0.0
        || jump_state.spring_lock_timer > 0.0
    {
        return false;
    }

    perform_super_jump(jump_state, velocity, dash_state.direction.x, true);
    dash_state.is_dashing = false;
    dash_state.timer = 0.0;
    return true;
}

fn try_dash_corner_correction(
    translation: &mut Vec3,
    player_size: Vec2,
    dash_state: &DashState,
    obstacles: &[(Vec3, Vec2)],
) -> bool {
    if !dash_state.is_dashing || dash_state.direction.y != 0.0 || dash_state.direction.x == 0.0 {
        return false;
    }

    for offset in 1..=DASH_CORNER_CORRECTION {
        let candidate = *translation + Vec3::new(0.0, offset as f32, 0.0);
        if can_use_collider(candidate, player_size, obstacles) {
            *translation = candidate;
            return true;
        }
    }

    false
}

fn try_trigger_corner_boost(
    corner_boost: &mut CornerBoostState,
    player_transform: &Transform,
    player_size: Vec2,
    velocity: &mut Velocity,
    dash_state: &mut DashState,
    state_machine: &mut PlayerStateMachine,
) {
    if !corner_boost.active
        || corner_boost.timer <= 0.0
        || !corner_boost.climb_jump_armed
        || corner_boost.passed_wall_top_after_arm
    {
        return;
    }

    let player_bottom = player_transform.translation.y - player_size.y / 2.0;
    let cleared_wall_top = player_bottom >= corner_boost.wall_top_y - CORNER_BOOST_TOP_TOLERANCE;
    if !cleared_wall_top {
        return;
    }

    corner_boost.passed_wall_top_after_arm = true;

    let boosted_speed = corner_boost.blocked_speed_x.abs() + CORNER_BOOST_BONUS_SPEED;
    let target_speed = corner_boost.direction * boosted_speed;
    if velocity.0.x.signum() != corner_boost.direction || velocity.0.x.abs() <= boosted_speed {
        velocity.0.x = target_speed;
    }

    if dash_state.is_dashing {
        dash_state.is_dashing = false;
        dash_state.timer = 0.0;
        if state_machine.current == PlayerState::Dash {
            transition_player_state(state_machine, PlayerState::Normal);
        }
    }

    corner_boost.clear();
}

fn try_upward_corner_correction(
    translation: &mut Vec3,
    player_size: Vec2,
    move_input: &MovementInput,
    obstacles: &[(Vec3, Vec2)],
) -> bool {
    let horizontal_preferences = if move_input.x > 0.0 {
        [1.0, -1.0]
    } else if move_input.x < 0.0 {
        [-1.0, 1.0]
    } else {
        [1.0, -1.0]
    };

    for offset in 1..=UPWARD_CORNER_CORRECTION {
        for direction in horizontal_preferences {
            let candidate = *translation + Vec3::new(offset as f32 * direction, 0.0, 0.0);
            if can_use_collider(candidate, player_size, obstacles) {
                *translation = candidate;
                return true;
            }
        }
    }

    false
}

fn find_climb_top_out_target(
    translation: Vec3,
    player_size: Vec2,
    wall_contact: WallContact,
    obstacles: &[(Vec3, Vec2)],
) -> Option<Vec3> {
    let player_half = player_size * 0.5;
    let player_bottom = translation.y - player_half.y;
    let player_left = translation.x - player_half.x;
    let player_right = translation.x + player_half.x;
    let wall_snap_tolerance = 1.5;

    for (ground_pos, ground_size) in obstacles {
        let ground_half = *ground_size * 0.5;
        let wall_top = ground_pos.y + ground_half.y;
        let wall_left = ground_pos.x - ground_half.x;
        let wall_right = ground_pos.x + ground_half.x;

        let is_adjacent = match wall_contact {
            WallContact::Left => (player_left - wall_right).abs() <= wall_snap_tolerance,
            WallContact::Right => (player_right - wall_left).abs() <= wall_snap_tolerance,
            WallContact::None => false,
        };

        if !is_adjacent || player_bottom < wall_top {
            continue;
        }

        let stand_x = match wall_contact {
            WallContact::Left => wall_right,
            WallContact::Right => wall_left,
            WallContact::None => translation.x,
        };
        let candidate = Vec3::new(stand_x, wall_top + player_half.y, translation.z);

        if can_use_collider(candidate, player_size, obstacles) {
            return Some(candidate);
        }
    }

    None
}

fn resolve_dash_direction(
    move_input: &MovementInput,
    grounded: &Grounded,
    crouching: &Crouching,
    facing: &Facing,
) -> Vec2 {
    let mut dash_dir = Vec2::new(move_input.x, move_input.y);

    if grounded.0 && crouching.0 {
        dash_dir.y = 0.0;
    }

    if dash_dir == Vec2::ZERO {
        dash_dir = Vec2::new(facing.0.signum().max(-1.0), 0.0);
        if dash_dir.x == 0.0 {
            dash_dir.x = 1.0;
        }
    }

    dash_dir.normalize_or_zero()
}

fn is_facing_wall(facing: &Facing, wall_contact: &WallContact) -> bool {
    match wall_contact {
        WallContact::Left => facing.0 < 0.0,
        WallContact::Right => facing.0 > 0.0,
        WallContact::None => false,
    }
}

fn transition_player_state(state_machine: &mut PlayerStateMachine, next_state: PlayerState) {
    if state_machine.current != next_state {
        state_machine.previous = state_machine.current;
        state_machine.current = next_state;
    }
}

fn try_consume_climb_jump(
    jump_state: &mut JumpState,
    in_climb_state: bool,
    wall_contact: &WallContact,
    move_input: &MovementInput,
    actions: &PlayerActionInput,
    facing: &mut Facing,
    velocity: &mut Velocity,
    wall_jump_timer: &mut WallJumpTimer,
    climb_stamina: &mut ClimbStamina,
    corner_boost: &mut CornerBoostState,
) -> bool {
    if jump_state.jump_buffer_timer <= 0.0
        || *wall_contact == WallContact::None
        || !actions.grab_held
        || climb_stamina.current <= CLIMB_STAMINA_MIN_TO_CLIMB
    {
        return false;
    }

    let wall_dir = match wall_contact {
        WallContact::Left => -1.0,
        WallContact::Right => 1.0,
        WallContact::None => 0.0,
    };

    let jumping_away = move_input.x != 0.0 && move_input.x.signum() == -wall_dir;
    let jump_started_from_recorded_wall = *wall_contact == corner_boost.wall_side;
    let is_corner_boost_jump = corner_boost.active
        && corner_boost.timer > 0.0
        && !corner_boost.climb_jump_armed
        && !corner_boost.climb_jump_used
        && jump_started_from_recorded_wall;
    let can_climb_jump = jumping_away
        || in_climb_state
        || is_facing_wall(facing, wall_contact)
        || is_corner_boost_jump;

    if !can_climb_jump {
        return false;
    }

    if jumping_away {
        velocity.0.x = -wall_dir * JUMP_AWAY_FROM_WALL;
        velocity.0.y = WALL_CLIMB_JUMP_FORCE_Y;
        wall_jump_timer.0 = WALL_KICK_LOCK;
        facing.0 = -wall_dir;
    } else if is_corner_boost_jump {
        velocity.0.y = WALL_CLIMB_JUMP_FORCE_Y;
        wall_jump_timer.0 = WALL_CLIMB_LOCK;
        facing.0 = wall_dir;
    } else {
        velocity.0.y = WALL_CLIMB_JUMP_FORCE_Y;
        velocity.0.x = 0.0;
        wall_jump_timer.0 = WALL_CLIMB_LOCK;
        facing.0 = wall_dir;
    }

    jump_state.jump_buffer_timer = 0.0;
    jump_state.source = JumpSource::Player;
    climb_stamina.current = (climb_stamina.current - CLIMB_STAMINA_JUMP_COST).max(0.0);

    if is_corner_boost_jump {
        corner_boost.climb_jump_armed = true;
        corner_boost.climb_jump_used = true;
        corner_boost.passed_wall_top_after_arm = false;
    }
    true
}

fn resolve_player_state(
    current_state: PlayerState,
    actions: &PlayerActionInput,
    facing: &Facing,
    wall_contact: &WallContact,
    wall_jump_timer: &WallJumpTimer,
    dash_state: &DashState,
    climb_top_out: &ClimbTopOutState,
    climb_stamina: &ClimbStamina,
    jump_state: &JumpState,
) -> PlayerState {
    if dash_state.is_dashing {
        PlayerState::Dash
    } else if climb_top_out.active {
        PlayerState::TopOut
    } else if jump_state.spring_lock_timer > 0.0 {
        PlayerState::Normal
    } else if current_state == PlayerState::Climb {
        let can_keep_climbing = actions.grab_held
            && *wall_contact != WallContact::None
            && wall_jump_timer.0 <= 0.0
            && climb_stamina.current > CLIMB_STAMINA_MIN_TO_CLIMB;

        if !can_keep_climbing {
            PlayerState::Normal
        } else {
            PlayerState::Climb
        }
    } else if actions.grab_held
        && *wall_contact != WallContact::None
        && is_facing_wall(facing, wall_contact)
        && wall_jump_timer.0 <= 0.0
        && climb_stamina.current > CLIMB_STAMINA_MIN_TO_CLIMB
    {
        PlayerState::Climb
    } else {
        PlayerState::Normal
    }
}

fn apply_state_end(
    previous_state: PlayerState,
    next_state: PlayerState,
    velocity: &mut Velocity,
    _dash_state: &DashState,
) {
    if previous_state == PlayerState::Dash && next_state != PlayerState::Dash {
        velocity.0 *= DASH_END_MULTIPLIER;
        if velocity.0.y < 0.0 {
            velocity.0.y *= 0.5;
        }
    }
}

pub fn cache_player_input(
    keyboard_input: Res<ButtonInput<KeyCode>>,
    mut query: Query<(&mut MovementInput, &mut PlayerActionInput, &mut Facing), With<Player>>,
) {
    if let Ok((mut move_input, mut actions, mut facing)) = query.get_single_mut() {
        let left =
            keyboard_input.pressed(KeyCode::KeyA) || keyboard_input.pressed(KeyCode::ArrowLeft);
        let right =
            keyboard_input.pressed(KeyCode::KeyD) || keyboard_input.pressed(KeyCode::ArrowRight);
        let up = keyboard_input.pressed(KeyCode::KeyW) || keyboard_input.pressed(KeyCode::ArrowUp);
        let down =
            keyboard_input.pressed(KeyCode::KeyS) || keyboard_input.pressed(KeyCode::ArrowDown);

        move_input.x = if right && !left {
            1.0
        } else if left && !right {
            -1.0
        } else {
            0.0
        };
        move_input.y = if up && !down {
            1.0
        } else if down && !up {
            -1.0
        } else {
            0.0
        };

        actions.jump_pressed = keyboard_input.just_pressed(KeyCode::Space)
            || keyboard_input.just_pressed(KeyCode::KeyZ);
        actions.jump_held =
            keyboard_input.pressed(KeyCode::Space) || keyboard_input.pressed(KeyCode::KeyZ);
        let dash_held =
            keyboard_input.pressed(KeyCode::KeyK) || keyboard_input.pressed(KeyCode::KeyC);
        let raw_dash_pressed = keyboard_input.just_pressed(KeyCode::KeyK)
            || keyboard_input.just_pressed(KeyCode::KeyC);
        if !dash_held {
            actions.dash_requires_release = false;
        }
        actions.dash_pressed = raw_dash_pressed && !actions.dash_requires_release;
        actions.grab_held =
            keyboard_input.pressed(KeyCode::KeyJ) || keyboard_input.pressed(KeyCode::KeyX);

        if move_input.x != 0.0 {
            facing.0 = move_input.x;
        }
    }
}

pub fn update_player_state_machine(
    mut query: Query<
        (
            &mut Velocity,
            &PlayerActionInput,
            &Facing,
            &WallContact,
            &WallJumpTimer,
            &DashState,
            &ClimbTopOutState,
            &ClimbStamina,
            &JumpState,
            &mut PlayerStateMachine,
        ),
        With<Player>,
    >,
) {
    if let Ok((
        mut velocity,
        actions,
        facing,
        wall_contact,
        wall_jump_timer,
        dash_state,
        climb_top_out,
        climb_stamina,
        jump_state,
        mut state_machine,
    )) = query.get_single_mut()
    {
        let next_state = resolve_player_state(
            state_machine.current,
            actions,
            facing,
            wall_contact,
            wall_jump_timer,
            dash_state,
            climb_top_out,
            climb_stamina,
            jump_state,
        );

        if state_machine.current != next_state {
            apply_state_end(state_machine.current, next_state, &mut velocity, dash_state);
            transition_player_state(&mut state_machine, next_state);
        }
    }
}

pub fn tick_timers(
    time: Res<Time<Fixed>>,
    mut q_jump: Query<(&Grounded, &mut JumpState), With<Player>>,
    mut q_slide: Query<&mut DashSlideState, With<Player>>,
    mut q_wall: Query<&mut WallJumpTimer>,
    mut q_dash: Query<&mut DashState>,
) {
    let dt = time.delta_secs();

    for (grounded, mut jump_state) in &mut q_jump {
        if jump_state.spring_lock_timer > 0.0 {
            jump_state.spring_lock_timer = (jump_state.spring_lock_timer - dt).max(0.0);
        }

        if grounded.0 {
            jump_state.jump_grace_timer = JUMP_GRACE_TIME;
            jump_state.super_jump_timer = 0.0;
            jump_state.fast_jump_active = false;
            jump_state.spring_lock_timer = 0.0;
            jump_state.source = JumpSource::None;
        } else if jump_state.jump_grace_timer > 0.0 {
            jump_state.jump_grace_timer = (jump_state.jump_grace_timer - dt).max(0.0);
        }

        if jump_state.jump_buffer_timer > 0.0 {
            jump_state.jump_buffer_timer = (jump_state.jump_buffer_timer - dt).max(0.0);
        }

        if jump_state.super_jump_timer > 0.0 {
            jump_state.super_jump_timer = (jump_state.super_jump_timer - dt).max(0.0);
        }
    }

    for mut timer in &mut q_wall {
        if timer.0 > 0.0 {
            timer.0 -= dt;
        }
    }

    for mut dash_state in &mut q_dash {
        if dash_state.is_dashing {
            dash_state.timer -= dt;
            if dash_state.timer <= 0.0 {
                dash_state.is_dashing = false;
            }
        }
    }

    for mut dash_slide in &mut q_slide {
        if dash_slide.timer > 0.0 {
            dash_slide.timer = (dash_slide.timer - dt).max(0.0);
        }
    }
}

pub fn update_crouch_state(
    mut query: Query<
        (
            &mut Transform,
            &Grounded,
            &MovementInput,
            &DashState,
            &DashSlideState,
            &PlayerStateMachine,
            &mut Crouching,
            &mut ColliderSize,
        ),
        (With<Player>, Without<Ground>),
    >,
    obstacles_query: Query<(&Transform, &Sprite), (With<Ground>, Without<Player>)>,
) {
    let obstacles: Vec<(Vec3, Vec2)> = obstacles_query
        .iter()
        .filter_map(|(transform, sprite)| {
            sprite.custom_size.map(|size| (transform.translation, size))
        })
        .collect();

    if let Ok((
        mut transform,
        grounded,
        move_input,
        dash_state,
        dash_slide,
        state_machine,
        mut crouching,
        mut collider_size,
    )) = query.get_single_mut()
    {
        let height_delta = PLAYER_COLLIDER_SIZE.y - CROUCH_COLLIDER_SIZE.y;

        if (state_machine.current == PlayerState::Climb
            || state_machine.current == PlayerState::TopOut)
            && crouching.0
        {
            crouching.0 = false;
            collider_size.0 = PLAYER_COLLIDER_SIZE;
            transform.translation.y += height_delta * 0.5;
        }

        let wants_crouch = state_machine.current != PlayerState::Climb
            && state_machine.current != PlayerState::TopOut
            && ((grounded.0 && move_input.y < 0.0 && !dash_state.is_dashing)
                || dash_slide.timer > 0.0);

        if wants_crouch && !crouching.0 {
            crouching.0 = true;
            collider_size.0 = CROUCH_COLLIDER_SIZE;
            transform.translation.y -= height_delta * 0.5;
        } else if !wants_crouch && crouching.0 {
            let expanded_translation =
                transform.translation + Vec3::new(0.0, height_delta * 0.5, 0.0);
            if can_use_collider(expanded_translation, PLAYER_COLLIDER_SIZE, &obstacles) {
                crouching.0 = false;
                collider_size.0 = PLAYER_COLLIDER_SIZE;
                transform.translation = expanded_translation;
            }
        }
    }
}

pub fn player_input(
    mut commands: Commands,
    asset_server: Res<AssetServer>,
    mut freeze_frames: ResMut<FreezeFrameState>,
    mut query: Query<
        (
            &mut Velocity,
            &mut JumpState,
            &Grounded,
            &WallContact,
            &mut WallJumpTimer,
            &mut DashState,
            &mut DashSlideState,
            &MovementInput,
            &mut PlayerActionInput,
            &mut Facing,
            &Crouching,
            &ClimbTopOutState,
            &mut PlayerStateMachine,
            &mut ClimbStamina,
            &mut CornerBoostState,
        ),
        With<Player>,
    >,
) {
    if let Ok((
        mut velocity,
        mut jump_state,
        grounded,
        wall_contact,
        mut wall_jump_timer,
        mut dash_state,
        mut dash_slide,
        move_input,
        mut actions,
        mut facing,
        crouching,
        climb_top_out,
        mut state_machine,
        mut climb_stamina,
        mut corner_boost,
    )) = query.get_single_mut()
    {
        if climb_top_out.active || state_machine.current == PlayerState::TopOut {
            actions.jump_pressed = false;
            actions.dash_pressed = false;
            return;
        }

        if actions.jump_pressed {
            jump_state.jump_buffer_timer = JUMP_BUFFER_TIME;
            actions.jump_pressed = false;
        }

        if grounded.0 && velocity.0.y <= 0.0 && !dash_state.is_dashing {
            dash_state.dashes_remaining = 1;
            climb_stamina.current = CLIMB_STAMINA_MAX;
            climb_stamina.low_flash_timer = 0.0;
        }

        if actions.dash_pressed && dash_state.dashes_remaining > 0 {
            play_dash_sfx(&mut commands, &asset_server);
            dash_state.is_dashing = true;
            dash_state.timer = DASH_DURATION;
            dash_state.dashes_remaining -= 1;
            dash_slide.timer = 0.0;
            dash_slide.direction = 0.0;
            if freeze_frames.timer <= 0.0 {
                freeze_frames.timer = DASH_FREEZE_TIME;
            }

            dash_state.direction = resolve_dash_direction(move_input, grounded, crouching, &facing);
            velocity.0 = dash_state.direction * DASH_SPEED;
            transition_player_state(&mut state_machine, PlayerState::Dash);
        }

        if try_consume_grounded_super_jump(
            &mut jump_state,
            &mut dash_state,
            grounded,
            move_input,
            &mut velocity,
        ) {
            transition_player_state(&mut state_machine, PlayerState::Normal);
            return;
        }

        if (dash_state.is_dashing || state_machine.current == PlayerState::Dash)
            && jump_state.jump_buffer_timer > 0.0
            && *wall_contact != WallContact::None
            && actions.grab_held
            && try_consume_climb_jump(
                &mut jump_state,
                false,
                wall_contact,
                move_input,
                &actions,
                &mut facing,
                &mut velocity,
                &mut wall_jump_timer,
                &mut climb_stamina,
                &mut corner_boost,
            )
        {
            dash_state.is_dashing = false;
            dash_state.timer = 0.0;
            dash_slide.timer = 0.0;
            dash_slide.direction = 0.0;
            transition_player_state(&mut state_machine, PlayerState::Normal);
            return;
        }

        if state_machine.current == PlayerState::Dash && dash_slide.timer <= 0.0 {
            return;
        }

        if try_consume_super_jump(&mut jump_state, &mut dash_slide, &mut velocity) {
            transition_player_state(&mut state_machine, PlayerState::Normal);
            return;
        }

        if state_machine.current == PlayerState::Climb
            && try_consume_climb_jump(
                &mut jump_state,
                true,
                wall_contact,
                move_input,
                &actions,
                &mut facing,
                &mut velocity,
                &mut wall_jump_timer,
                &mut climb_stamina,
                &mut corner_boost,
            )
        {
            transition_player_state(&mut state_machine, PlayerState::Normal);
            return;
        }

        if try_consume_ground_jump(&mut jump_state, grounded, &mut velocity) {
            return;
        }

        if try_consume_climb_jump(
            &mut jump_state,
            false,
            wall_contact,
            move_input,
            &actions,
            &mut facing,
            &mut velocity,
            &mut wall_jump_timer,
            &mut climb_stamina,
            &mut corner_boost,
        ) {
            transition_player_state(&mut state_machine, PlayerState::Normal);
            return;
        }

        if jump_state.jump_buffer_timer > 0.0
            && jump_state.spring_lock_timer <= 0.0
            && !grounded.0
            && *wall_contact != WallContact::None
        {
            let wall_dir = match wall_contact {
                WallContact::Left => -1.0,
                WallContact::Right => 1.0,
                WallContact::None => 0.0,
            };

            if move_input.x == 0.0 {
                velocity.0.x = -wall_dir * WALL_NEUTRAL_FORCE.x;
                velocity.0.y = WALL_NEUTRAL_FORCE.y;
                wall_jump_timer.0 = WALL_NEUTRAL_LOCK;
            } else {
                velocity.0.x = -wall_dir * WALL_KICK_FORCE.x;
                velocity.0.y = WALL_KICK_FORCE.y;
                wall_jump_timer.0 = WALL_KICK_LOCK;
            }

            jump_state.jump_buffer_timer = 0.0;
            jump_state.super_jump_timer = 0.0;
            jump_state.fast_jump_active = false;
            jump_state.source = JumpSource::Player;
        }
    }
}

pub fn trigger_fall_death(
    mut commands: Commands,
    asset_server: Res<AssetServer>,
    active_room: Res<ActiveRoom>,
    mut death_sequence: ResMut<DeathSequence>,
    player_query: Query<&Transform, With<Player>>,
) {
    if death_sequence.active() {
        return;
    }

    let Ok(player_transform) = player_query.get_single() else {
        return;
    };

    let _current_respawn = active_room.respawn_point;
    if player_transform.translation.y < DEATH_THRESHOLD && death_sequence.start() {
        play_death_sfx(&mut commands, &asset_server);
    }
}

pub fn apply_physics(
    mut query: Query<
        (
            &mut Velocity,
            &JumpState,
            &PlayerStateMachine,
            &WallContact,
            &Grounded,
            &WallJumpTimer,
            &DashState,
            &MovementInput,
            &PlayerActionInput,
            &Crouching,
            &ClimbTopOutState,
            &mut ClimbStamina,
        ),
        With<Player>,
    >,
    time: Res<Time<Fixed>>,
) {
    if let Ok((
        mut velocity,
        jump_state,
        state_machine,
        wall_contact,
        grounded,
        wall_jump_timer,
        dash_state,
        move_input,
        actions,
        crouching,
        climb_top_out,
        mut climb_stamina,
    )) = query.get_single_mut()
    {
        let dt = time.delta_secs();

        match state_machine.current {
            PlayerState::Dash => {
                velocity.0 = dash_state.direction * DASH_SPEED;
            }
            PlayerState::TopOut => {
                velocity.0 = Vec2::ZERO;
            }
            PlayerState::Climb => {
                velocity.0.x = 0.0;
                let mut climb_dir = 0.0;
                if move_input.y > 0.0 {
                    climb_dir += 1.0;
                }
                if move_input.y < 0.0 {
                    climb_dir -= 1.0;
                }

                let drain_rate = if climb_dir > 0.0 {
                    CLIMB_STAMINA_UP_DRAIN_PER_SEC
                } else {
                    CLIMB_STAMINA_HOLD_DRAIN_PER_SEC
                };
                climb_stamina.current = (climb_stamina.current - drain_rate * dt).max(0.0);

                if climb_stamina.current <= CLIMB_STAMINA_MIN_TO_CLIMB {
                    velocity.0.y = 0.0;
                } else {
                    velocity.0.y = climb_dir * WALL_CLIMB_SPEED;
                }
            }
            PlayerState::Normal => {
                if wall_jump_timer.0 <= 0.0 {
                    let current_speed = velocity.0.x;

                    if jump_state.super_jump_timer > 0.0 && !grounded.0 {
                        if move_input.x != 0.0
                            && current_speed != 0.0
                            && current_speed.signum() != move_input.x.signum()
                        {
                            velocity.0.x = move_towards(
                                current_speed,
                                move_input.x * MAX_RUN_SPEED,
                                SUPER_JUMP_TURN_FRICTION * dt,
                            );
                        } else {
                            velocity.0.x = current_speed;
                        }
                    } else if jump_state.spring_lock_timer > 0.0 && !grounded.0 {
                        velocity.0.x = current_speed;
                    } else {
                        let target_speed = if crouching.0 {
                            0.0
                        } else {
                            move_input.x * MAX_RUN_SPEED
                        };

                        let (accel, friction, turn_friction) = if grounded.0 {
                            (GROUND_ACCEL, GROUND_FRICTION, GROUND_TURN_FRICTION)
                        } else {
                            (AIR_ACCEL, AIR_FRICTION, AIR_TURN_FRICTION)
                        };

                        let final_accel = if move_input.x != 0.0 {
                            if current_speed.signum() != move_input.x.signum()
                                && current_speed != 0.0
                            {
                                turn_friction
                            } else {
                                accel
                            }
                        } else {
                            friction
                        };

                        velocity.0.x = move_towards(current_speed, target_speed, final_accel * dt);
                    }
                } else {
                    velocity.0.x = move_towards(velocity.0.x, 0.0, AIR_FRICTION * 0.5 * dt);
                }

                let gravity_multiplier = if jump_state.super_jump_timer > 0.0 {
                    if velocity.0.y.abs() < SUPER_JUMP_APEX_THRESHOLD {
                        SUPER_JUMP_APEX_GRAVITY_MULTIPLIER
                    } else if velocity.0.y < 0.0 {
                        SUPER_JUMP_FALL_MULTIPLIER
                    } else if actions.jump_held {
                        0.9
                    } else {
                        1.1
                    }
                } else if jump_state.source == JumpSource::Spring {
                    if velocity.0.y > 0.0 {
                        SPRING_ASCEND_GRAVITY_MULTIPLIER
                    } else if velocity.0.y < 0.0 && actions.jump_held {
                        SPRING_FALL_HELD_GRAVITY_MULTIPLIER
                    } else {
                        SPRING_FALL_GRAVITY_MULTIPLIER
                    }
                } else if velocity.0.y > 0.0 {
                    if actions.jump_held {
                        PLAYER_JUMP_HELD_GRAVITY_MULTIPLIER
                    } else {
                        PLAYER_JUMP_RELEASED_GRAVITY_MULTIPLIER
                    }
                } else if actions.jump_held && velocity.0.y < 0.0 {
                    if velocity.0.y.abs() < HALF_GRAVITY_THRESHOLD {
                        PLAYER_FALL_HELD_GRAVITY_MULTIPLIER
                    } else {
                        PLAYER_FALL_GRAVITY_MULTIPLIER
                    }
                } else {
                    PLAYER_FALL_GRAVITY_MULTIPLIER
                };

                velocity.0.y -= GRAVITY * gravity_multiplier * dt;

                if !grounded.0 && velocity.0.y < -MAX_FALL_SPEED {
                    velocity.0.y = -MAX_FALL_SPEED;
                }

                if *wall_contact != WallContact::None
                    && velocity.0.y < 0.0
                    && !grounded.0
                    && wall_jump_timer.0 <= 0.0
                    && !climb_top_out.active
                    && velocity.0.y < -WALL_SLIDE_SPEED
                {
                    velocity.0.y = -WALL_SLIDE_SPEED;
                }
            }
        }
    }
}

pub fn player_movement(
    mut param_set: ParamSet<(
        Query<
            (
                &mut Transform,
                &mut Velocity,
                &JumpState,
                &mut Grounded,
                &mut WallContact,
                &ColliderSize,
                &mut Crouching,
                &mut DashState,
                &mut DashSlideState,
                &mut ClimbTopOutState,
                &mut CornerBoostState,
                &mut PlayerStateMachine,
                &MovementInput,
                &ClimbStamina,
            ),
            With<Player>,
        >,
        Query<(&Transform, &Sprite), With<Ground>>,
    )>,
    time: Res<Time<Fixed>>,
) {
    let obstacles: Vec<(Vec3, Vec2)> = param_set
        .p1()
        .iter()
        .filter_map(|(transform, sprite)| {
            sprite.custom_size.map(|size| (transform.translation, size))
        })
        .collect();

    if let Ok((
        mut player_transform,
        mut velocity,
        _jump_state,
        mut grounded,
        mut wall_contact,
        collider_size,
        mut crouching,
        mut dash_state,
        mut dash_slide,
        mut climb_top_out,
        mut corner_boost,
        mut state_machine,
        move_input,
        _climb_stamina,
    )) = param_set.p0().get_single_mut()
    {
        let player_size = collider_size.0;
        let delta_seconds = time.delta_secs();
        let was_grounded = grounded.0;

        if climb_top_out.active {
            if corner_boost.active {
                corner_boost.timer = (corner_boost.timer - delta_seconds).max(0.0);
                if corner_boost.timer <= 0.0 {
                    corner_boost.clear();
                }
            }

            climb_top_out.timer = (climb_top_out.timer - delta_seconds).max(0.0);
            let progress = if climb_top_out.duration <= 0.0 {
                1.0
            } else {
                1.0 - climb_top_out.timer / climb_top_out.duration
            };
            player_transform.translation = climb_top_out
                .start
                .lerp(climb_top_out.target, progress.clamp(0.0, 1.0));
            velocity.0 = Vec2::ZERO;
            grounded.0 = false;

            if climb_top_out.timer <= 0.0 {
                player_transform.translation = climb_top_out.target;
                climb_top_out.active = false;
                grounded.0 = true;
                *wall_contact = WallContact::None;
                transition_player_state(&mut state_machine, PlayerState::Normal);
                try_trigger_corner_boost(
                    &mut corner_boost,
                    &player_transform,
                    player_size,
                    &mut velocity,
                    &mut dash_state,
                    &mut state_machine,
                );
            } else {
                *wall_contact = WallContact::None;
                transition_player_state(&mut state_machine, PlayerState::TopOut);
            }
            return;
        }

        if corner_boost.active {
            corner_boost.timer = (corner_boost.timer - delta_seconds).max(0.0);
            if corner_boost.timer <= 0.0 {
                corner_boost.clear();
            }
        }

        grounded.0 = false;

        player_transform.translation.x += velocity.0.x * delta_seconds;
        for (ground_pos, ground_size) in &obstacles {
            if check_collision(
                player_transform.translation,
                player_size,
                *ground_pos,
                *ground_size,
            ) {
                if !try_dash_corner_correction(
                    &mut player_transform.translation,
                    player_size,
                    &dash_state,
                    &obstacles,
                ) {
                    if velocity.0.x > 0.0 {
                        player_transform.translation.x =
                            ground_pos.x - ground_size.x / 2.0 - player_size.x / 2.0;
                    } else if velocity.0.x < 0.0 {
                        player_transform.translation.x =
                            ground_pos.x + ground_size.x / 2.0 + player_size.x / 2.0;
                    }

                    if !corner_boost.active && velocity.0.x.abs() >= CORNER_BOOST_MIN_BLOCKED_SPEED {
                        corner_boost.active = true;
                        corner_boost.timer = CORNER_BOOST_WINDOW;
                        corner_boost.blocked_speed_x = velocity.0.x;
                        corner_boost.direction = velocity.0.x.signum();
                        corner_boost.wall_side = if velocity.0.x > 0.0 {
                            WallContact::Right
                        } else {
                            WallContact::Left
                        };
                        corner_boost.wall_top_y = ground_pos.y + ground_size.y / 2.0;
                        corner_boost.climb_jump_armed = false;
                        corner_boost.climb_jump_used = false;
                        corner_boost.passed_wall_top_after_arm = false;
                    }

                    velocity.0.x = 0.0;
                }
            }
        }

        *wall_contact = WallContact::None;
        let sensor_margin = 1.0;
        for (ground_pos, ground_size) in &obstacles {
            let left_check_pos = player_transform.translation - Vec3::new(sensor_margin, 0.0, 0.0);
            if check_collision(left_check_pos, player_size, *ground_pos, *ground_size) {
                *wall_contact = WallContact::Left;
            }
            let right_check_pos = player_transform.translation + Vec3::new(sensor_margin, 0.0, 0.0);
            if check_collision(right_check_pos, player_size, *ground_pos, *ground_size) {
                *wall_contact = WallContact::Right;
            }
        }

        player_transform.translation.y += velocity.0.y * delta_seconds;
        for (ground_pos, ground_size) in &obstacles {
            if check_collision(
                player_transform.translation,
                player_size,
                *ground_pos,
                *ground_size,
            ) {
                if velocity.0.y > 0.0 {
                    if !try_upward_corner_correction(
                        &mut player_transform.translation,
                        player_size,
                        &MovementInput {
                            x: velocity.0.x.signum(),
                            y: velocity.0.y.signum(),
                        },
                        &obstacles,
                    ) {
                        player_transform.translation.y =
                            ground_pos.y - ground_size.y / 2.0 - player_size.y / 2.0;
                        velocity.0.y = 0.0;
                    }
                } else if velocity.0.y < 0.0 {
                    player_transform.translation.y =
                        ground_pos.y + ground_size.y / 2.0 + player_size.y / 2.0;
                    grounded.0 = true;
                    velocity.0.y = 0.0;
                }
            }
        }

        try_trigger_corner_boost(
            &mut corner_boost,
            &player_transform,
            player_size,
            &mut velocity,
            &mut dash_state,
            &mut state_machine,
        );

        if corner_boost.active && corner_boost.climb_jump_armed {
            let failed_on_recorded_wall = *wall_contact == corner_boost.wall_side && velocity.0.y <= 0.0;
            if grounded.0 || failed_on_recorded_wall {
                corner_boost.clear();
            }
        }

        if state_machine.current == PlayerState::Climb && move_input.y > 0.0 {
            if let Some(target) = find_climb_top_out_target(
                player_transform.translation,
                player_size,
                *wall_contact,
                &obstacles,
            ) {
                climb_top_out.active = true;
                climb_top_out.duration = 0.12;
                climb_top_out.timer = climb_top_out.duration;
                climb_top_out.start = player_transform.translation;
                climb_top_out.target = target;
                velocity.0 = Vec2::ZERO;
                grounded.0 = false;
                *wall_contact = WallContact::None;
                transition_player_state(&mut state_machine, PlayerState::TopOut);
                return;
            }
        }

        if grounded.0
            && !was_grounded
            && dash_state.is_dashing
            && dash_state.direction.x != 0.0
            && dash_state.direction.y < 0.0
        {
            velocity.0 = Vec2::ZERO;
            dash_state.is_dashing = false;
            dash_state.timer = 0.0;
            dash_slide.timer = DASH_SLIDE_WINDOW;
            dash_slide.direction = dash_state.direction.x.signum();
            velocity.0.y = 0.0;
            velocity.0.x = dash_slide.direction * (DASH_SPEED * DASH_SLIDE_SPEED_MULTIPLIER);
            crouching.0 = true;
            transition_player_state(&mut state_machine, PlayerState::Normal);
        }
    }
}
